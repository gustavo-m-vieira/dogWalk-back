...
import { makeConfirmDogTripController } from '../factories';
...
export default (router: Router): void => {
  router.post(
    '/trips/:tripId/dogs/:dogId/caught',
    (req, res, next) => new AuthenticationMiddleware(process.env.JWT_KEY).execute(req, res, next),
    (req, res, next) => new AuthorizationMiddleware([UserRoleEnum.WALKER]).execute(req, res, next),
    adaptRoute(makeConfirmDogTripController())
  );
};
